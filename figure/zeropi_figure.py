...
eval0 = scq.read("data/zeropi_0_specdata_truc=1000_3ncut.h5").energy_table
n_theta0 = (2 * np.pi * scq.read("data/zeropi_0_n_theta_truc=1000_3ncut.h5").matrixelem_table)
n_phi0 = (2 * np.pi * scq.read("data/zeropi_0_n_phi_truc=1000_3ncut.h5").matrixelem_table)
...
zero_pi = scq.ZeroPi(
grid=phi_grid, EJ=EJ, EL=EL, ECJ=E_CJ, EC=E_C, dEJ=0.1, ng=0.0, flux=0.0, ncut=30
)
...
def latex_symbol_inkscape(EL, EJ, E_CJ, E_C, e_flux, e_ng):
...
def eval_vs_ng(Fontsize_legend, e_flux, e_ng):
    ...
    plt.savefig(
    "pdf_before_inkscape/1_energy.pdf",
    dpi=300,
    bbox_inches="tight",
    pad_inches=0.02,
)

eval_vs_ng(Fontsize_legend, e_flux, e_ng)
...
plt.savefig(
    "pdf_before_inkscape/1_potential2d.pdf",
    dpi=300,
    bbox_inches="tight",
    pad_inches=0.02,
)
...
ax1.invert_yaxis()
plt.savefig(
    "pdf_before_inkscape/1_double_well.pdf",
    dpi=300,
    bbox_inches="tight",
    pad_inches=0.02,
)
        ...
        if state_i in state_pop:
            for j, state_j in enumerate(hspace):
                if i < j and np.abs(n_op[i, j] / (2 * np.pi)) > thresh:
                    w_trans.append(
                        np.round(np.abs(evals[state_i] - evals[state_j]), 8))
                    trans_int.append((state_i, state_j))
                    n_trans.append(
                        np.round(np.abs(n_op[i, j] / (2 * np.pi)), 3))
...
plt.savefig("pdf_before_inkscape/xgate_ntheta.pdf",
            dpi=300,
            bbox_inches="tight",
            pad_inches=0.02)
...
plt.savefig("pdf_before_inkscape/xgate_nphi_test.pdf",
            dpi=300,
            bbox_inches="tight",
            pad_inches=0.02)
...
plt.savefig(
    "pdf_before_inkscape/params_1q.pdf",
    dpi=300,
    bbox_inches="tight",
    pad_inches=0.02,
)
...
for j in range(2):
    ...
    ax[j, 1].set_xticklabels([])
# Save figure
plt.savefig("pdf_before_inkscape/params_2q.pdf",
            dpi=300,
            bbox_inches="tight",
            pad_inches=0.02)
############################################################################

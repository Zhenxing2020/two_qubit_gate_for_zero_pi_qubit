...
def get_fidelity_2q(n_truc=50):
    ...
    option_ideal =qt.Options(max_step=max_step_ideal, 
                             nsteps=nsteps_ideal, num_cpus=16)  
    option_noisy =qt.Options(max_step=max_step_noisy, 
                             nsteps=nsteps_noisy, num_cpus=16)  
    ...
    print(f'Ideal: max_step = {option_ideal.max_step},'
          + f' nsteps = {option_ideal.nsteps}, num_cpus = {option_ideal.num_cpus}')
    print(f'Noisy: max_step = {option_noisy.max_step},'
          + f' nsteps = {option_noisy.nsteps}, num_cpus = {option_noisy.num_cpus}')
    ...
    if calculate_noise: # Noisey fidelity       
        ...
        c_op_list = ut.construct_c_ops_2q(dim_0, dim_1, n_theta0, 
                        n_theta1, gamma_dephase_02_q0, gamma_dephase_02_q1, 
                        eket_tot, Gamma_decay_q0, Gamma_decay_q1, filter_ratio=filter_ratio)     
        print(f'filter_ratio = {filter_ratio}, np.shape(c_op_list) = {np.shape(c_op_list)}')     

        c_op_super = [qt.to_super(c_op_list[i]) for i in range(len(c_op_list))]
        c_op_super_sum = [ sum(c_op_super) ]

        if cz_run:            
            # np.savez(f'data/collapse_ops_truc1={truc_one_qubit}_'+
            #          f'n={n_truc}_{t1_tphi_other}us.npz', c_op_list=c_op_list)
...
if __name__ == '__main__':
    ...
    f_list = []
    n_truc_list = [45, 46, 50] # np.arange(50, 401, 50) # [225, 250, 275, 300, 325, 350, 375, 400] 

"""
Run a 21-point sweep of ECJ2 over +/- 5% of its nominal value and save all
sweep points into a single npz via save_two_qubit_sweep.

Usage
-----
    conda activate zp2q
    python run_ecj2_sweep.py

Output
------
    data/ECJ2_sweep/two_qubit_sweep_data.npz   (combined, evecs dropped)
    data/ECJ2_sweep/summary_ECJ2=<value>.txt    (one per sweep point)
"""
...
# 21 points spanning +/- 5% of nominal ECJ2, swept linearly in the underlying
# junction capacitance CJ2 (ECJ2 ~ 1/CJ2, and capacitance is the additive
# physical quantity), same convention as the EC2 sweep.
nominal_ECJ2 = params["ECJ2"]
nominal_CJ2 = ghz_to_farad(nominal_ECJ2)
sweep_values = farad_to_ghz(np.linspace(max_entent[0] * nominal_CJ2, max_entent[1] * nominal_CJ2, n_pts))
print(f"Nominal ECJ2 = {nominal_ECJ2}")
print(f"Sweeping ECJ2 over {sweep_values[0]:.5f} .. {sweep_values[-1]:.5f} ({len(sweep_values)} points)")
...
folder_save = Path(DATA_FOLDER, "ECJ2_sweep")
save_two_qubit_sweep(params, "ECJ2", sweep_values, folder_save,

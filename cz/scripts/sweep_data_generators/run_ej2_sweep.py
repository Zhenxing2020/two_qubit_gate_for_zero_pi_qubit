"""
Run a 21-point sweep of EJ2 over +/- 5% of its nominal value and save all
sweep points into a single npz via save_two_qubit_sweep.

Usage
-----
    conda activate zp2q
    python run_ej2_sweep.py

Output
------
    data/EJ2_sweep/two_qubit_sweep_data.npz   (combined, evecs dropped)
    data/EJ2_sweep/summary_EJ2=<value>.txt    (one per sweep point)
"""
...
# 21 points spanning +/- 5% of nominal EJ2, swept directly in GHz. Unlike the
# capacitive/inductive energies, EJ2 is linear in the physical junction
# critical current (EJ = hbar*Ic/2e), so no unit conversion is needed before
# linspacing.
nominal_EJ2 = params["EJ2"]
sweep_values = np.linspace(max_entent[0] * nominal_EJ2, max_entent[1] * nominal_EJ2, n_pts)
print(f"Nominal EJ2 = {nominal_EJ2}")
print(f"Sweeping EJ2 over {sweep_values[0]:.5f} .. {sweep_values[-1]:.5f} ({len(sweep_values)} points)")
...
folder_save = Path(DATA_FOLDER, "EJ2_sweep")
save_two_qubit_sweep(params, "EJ2", sweep_values, folder_save,

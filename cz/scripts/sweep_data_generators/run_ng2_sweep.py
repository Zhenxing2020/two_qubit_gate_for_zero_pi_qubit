"""
Run a 21-point sweep of ng2 over a symmetric log-spaced range and save all
sweep points into a single npz via save_two_qubit_sweep.

Usage
-----
    conda activate zp2q
    python run_ng2_sweep.py

Output
------
    data/ng2_sweep/two_qubit_sweep_data.npz   (combined, evecs dropped)
    data/ng2_sweep/summary_ng2=<value>.txt    (one per sweep point)
"""
...
bound = 1e-02  # sweep range: -bound .. +bound
...
print(f"Nominal ng2 = {params['ng2']}")
print(f"Sweeping ng2 over {sweep_values[0]:.2e} .. {sweep_values[-1]:.2e} ({len(sweep_values)} points, log-spaced about 0)")
...
folder_save = Path(DATA_FOLDER, "ng2_sweep")
save_two_qubit_sweep(params, "ng2", sweep_values, folder_save,

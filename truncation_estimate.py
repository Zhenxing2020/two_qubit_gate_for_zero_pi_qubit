...
def _all_path_to_core_parallel(p):
    return all_path_to_core(*p)
...
def make_leakage_df(core_states, drive_term, evals, wd, A, G=None, labels = None, n_cpu=4):
    ...
    for target, (shortest_path_len, shortest_path) in pool.imap_unordered(_shortest_path_to_core_parallel, params):
        entry = {}
        entry["i"] = target
...
if __name__ == "__main__":
    ...
    states = trunc_by_graph_estimate(200, core_states, drive_term, eval_tot, wd, A, labels=hspace_full)

    print(states)


    dim = 10
    print(f'lowest {len(states)} dressed states :')
    data = states

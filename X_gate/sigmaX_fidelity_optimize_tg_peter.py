...
def fidelity_de():
    ...
    # for jdx, tg in tqdm(enumerate(x0_vec[:,0])):
        ...
        print(f'\nlog of gate error (truc={len(hspace_charge)}) = ')
        ...
        print(f'\ndrive_param (truc={len(hspace_charge)}) = ')
...
if __name__ == '__main__':
    ...
    amp1_bounds, amp2_bounds, detune1_bounds,  detune2_bounds = [(0, 0.3), (0., 0.3), (-1, 1), (-1, 1)]
    # amp1_bounds, amp2_bounds, detune1_bounds,  detune2_bounds = [(0, 0.1), (0, 0.3), (-0.8, -0.2), (-0.8, -0.2)]
    tg_bound = (-0.01, 0.01)
    tg_vec = np.arange(20, 170, step=20).tolist()
    # params = np.array([
...
# [100, 0.202994, 0.113049, 0.343961, 0.356533] ,
    ...
    # truc1, truc_full = 150, 300 # theta
    truc1, truc_full = 30, 30 # phi
    ...
    [H0, drive_term, w_trans_1, w_trans_2, hspace_charge] = ut.zero_pi_initialize_peter(drive_phi, drive_theta, truncation=truc1,)
    [H0_full, drive_full, _, _, _] = ut.zero_pi_initialize_peter(drive_phi, drive_theta, truncation=truc_full,)
    hspace_charge = np.arange(truc1).tolist()

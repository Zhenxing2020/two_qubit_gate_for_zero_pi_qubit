...
def xgate_fidelity_decay_all():
    ...
    drive_phi, drive_theta, n_full = True, False, 1000 # 50 states →12 workers, (100 states/40 workers, 200/160). 
    t1 = 3 # μs
    tg_list = np.arange(19).tolist() # # [1, 5, 9, 13, 17 ] #   ## 18 for theta, 19 for phi     
    ...
    if drive_theta:
        max_step_ideal, max_step_noisy = 3e-4, 3e-4 # Set max_step to 0 for parallel execution
    else:
